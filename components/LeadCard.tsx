...
const LeadCard: React.FC<LeadCardProps> = ({ lead, onClick, index = 0 }) => {
  ...
  return (
    <motion.div
      ...
      className={`group relative bg-[#1C1F26] border border-[#2D323B] rounded-2xl p-5 cursor-grab active:cursor-grabbing transition-all hover:border-[#D4AF37]/50 shadow-2xl ${isPremium ? 'ring-1 ring-[#D4AF37]/30 bg-gradient-to-br from-[#1C1F26] to-[#D4AF37]/5' : ''
    ...
    >
      ...
      <div className="flex justify-between items-start mb-3">
        <div className="flex flex-wrap gap-1.5">
          {isPremium && (
            <span className="bg-[#D4AF37]/10 text-[#D4AF37] text-[9px] font-black px-2.5 py-1 rounded-lg flex items-center gap-1.5 border border-[#D4AF37]/20">
              <span className="w-1 h-1 bg-[#D4AF37] rounded-full animate-pulse shadow-[0_0_8px_#D4AF37]"></span>
              PREMIUM
            </span>
          )}
          {urgent && (
            <span className="bg-status-error/10 text-status-error text-[9px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
              <ShieldAlert size={10} />
              URGENT
            </span>
          )}
      ...
      {/* Main Info */}
      <h3 className="font-black text-white text-sm mb-1 line-clamp-1 group-hover:text-[#D4AF37] transition-colors tracking-tight">
        {companyName}
      </h3>

      <div className="flex items-center gap-3 text-[10px] text-[#9E9E96] mb-5 font-black uppercase tracking-[0.15em]">
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <div className="bg-white/2 p-2.5 rounded-xl border border-white/5 group-hover:bg-white/5 transition-all">
          <p className="text-[9px] text-[#6B6B63] font-black uppercase tracking-widest mb-1">Potentiel</p>
          <p className="text-xs font-black text-white tracking-tighter">{getPotentialLabel()}</p>
        </div>
        <div className="bg-white/2 p-2.5 rounded-xl border border-white/5 group-hover:bg-white/5 transition-all">
          <div className="flex justify-between items-center mb-1.5">
            <p className="text-[9px] text-[#6B6B63] font-black uppercase tracking-widest">Score V-E</p>
            <span className="text-[10px] font-black text-[#D4AF37]">{score}</span>
          </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-white/5">
        <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest">
          ...
          {daysSinceContact !== null && daysSinceContact > 2 && isPremium && (
            <span className="text-[#EF4444] animate-pulse">CRITIQUE !</span>
          )}
        </div>
        ...
        <div className="flex -space-x-1.5 focus-within:z-10">
          <img
            ...
            className="w-5 h-5 rounded-full border border-white shadow-sm hover:scale-110 transition-transform cursor-help"
